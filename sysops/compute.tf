locals {
  test_solr_cluster_instance_count = 1
  test_solr_cluster_name_prefix = "test-stage"
  test_solr_cluster_zone = "us-central1-c"
  test_solr_cluster_machine_type = "n4-standard-2"
  ...
  test_solr_cluster_umig_name = "lb-umig-usc1g-c-test-stage-tf"
}


resource "google_compute_instance" "Instance_test-stage-instances" {
...
resource "google_compute_instance_group" "lb_umig_usc1g_c_solr_test_stage_tf" {
  ...
  zone = local.test_solr_cluster_zone
  project   = "${var.gcp_project}"
  instances = [ for x in google_compute_instance.Instance_test-stage-instances: x.self_link ]
  named_port {
    name = "tcp9100"
    port = "9100"
  }
